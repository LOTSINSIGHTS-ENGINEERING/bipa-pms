...
const NewValueModal = observer(() => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Updated `statements` to match the expected type in `IValue`
    const updatedValue: IValue = {
      ...value,
      statements: statements.map(({ statement }) => ({ statement, rating: 0 })),
    };
  
    if (!me) return;
    await create(updatedValue);
  ...
  const create = async (value: IValue) => {
    try {
      await api.value.create(value);
    } catch (error) {
  ...
  const onCancel = () => {
    ...
    hideModalFromId(MODAL_NAMES.THREE_SIXTY_FEEDBACK.CREATE_VALUE_MODAL);
  };
  ...
  return (
    ...
    >
      <button
        ...
        data-uk-close
      ></button>
      <h3 className="uk-modal-title">New Value</h3>
