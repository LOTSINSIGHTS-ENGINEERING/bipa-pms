...
const EditLeadershipModal = observer(() => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!me) return;
    await update(updatedLeadership);
    setLoading(false);
    onCancel();
  };

  const update = async (leadership: ILeadership) => {
    try {
      await api.leadership.update(leadership);
    } catch (error) {
  ...
  useEffect(() => {
    if (store.leadership.selected) {
      setLeadership({ ...store.leadership.selected });
      setStatements(
        store.leadership.selected.statements.map((statement) => ({
      ...
      setStatements(
        defaultLeadership.statements.map((statement) => ({
          id: statement.statement,
          statement: statement.statement,
          rating: statement.rating,
          editing: false,
      ...
      hideModalFromId(MODAL_NAMES.THREE_SIXTY_FEEDBACK.EDIT_LEADERSHIP_MODAL);
    }
  }, [store.leadership, store.leadership.selected]);
  ...
  return (
    <div className="user-modal uk-modal-dialog uk-modal-body uk-margin-auto-vertical" data-uk-overflow-auto>
      ...
      <h3 className="uk-modal-title">Edit Leadership</h3>
...
export default EditLeadershipModal;
