...
export const useAverageLeaderRating = () => {
  const { api, store } = useAppContext();

  const [description, setDescription] = useState("");
  const [loading, setLoading] = useState<Boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [leadershipValueScore, setLeadershipValueScore] = useState<number>(0); // Initialize with 0

  const valueRatings = useMemo(() => {
    return store.leadershipRating.all.map((rating) => rating.asJson.criteria);
  ...
  useEffect(() => {
    setLeadershipValueScore(calculateTotalValueScore(totalAverageScores)); // Set valueScore as number
  }, [totalAverageScores]);
  ...
  return { leadershipValueScore, loading, error };
