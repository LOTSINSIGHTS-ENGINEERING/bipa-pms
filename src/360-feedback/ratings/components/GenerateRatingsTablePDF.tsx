...
const GenerateRatingsTablePDF = (data: ILeadershipRating[]) => {
  ...
  const createPDF = async () => {
    ...
    const tableData = data.map(rating => {
      ...
      return [
        { text: new Date(rating.date).toLocaleDateString(), style: 'tableCell' },
        { text: getUserName(Object.keys(rating.criteria)[0]), style: 'tableCell' },
        ...
        { text: criteria ? criteria.additionalComment : "", style: 'tableCell' }
      ];
    });
...
export default GenerateRatingsTablePDF;
