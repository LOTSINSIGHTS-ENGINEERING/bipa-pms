...
const RatingsOverview = observer(() => {
  const { api, store } = useAppContext();
  const me = store.auth.meJson?.uid;
  const valueRatings = store.valueRating.all.filter(
    (value) => value.asJson.rateeId === me &&  value.asJson.adminApproval !== "Approved for Re-rating" && value.asJson.resubmission!=="To be rated" && value.asJson.resubmission!=="Closed" 
  );
  ...
  const leadershipRatings = store.leadershipRating.all
    ...
    console.log("leadership", leadershipRatings );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      console.log("my description new" + descriptions);
  ...
  return (
    <>
      ...
      <div className="doughnuts">
        ...
        <div className="doughnut-block">
          <h4 style={{ color: "#838CA7" }}>Professional Competencies 11</h4>
          <TemplatesDoughnut />
        </div>
      </div>
